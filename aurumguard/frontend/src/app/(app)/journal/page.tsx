...
export default function Journal() {
  ...
  return (
    <div>
      ...
      <Section title="Event log (latest 200)"><div className="scroll-x"><table className="table"><thead><tr><th>Time</th><th>Kind</th><th>Ref</th><th>Detail</th></tr></thead><tbody>{data.events.map((e, i) => <tr key={i}><td>{fmtTime(e.ts, tz)}</td><td>{e.kind}</td><td>{e.ref_id}</td><td className="text-xs">{summarize(e.detail, tz, 220)}</td></tr>)}</tbody></table></div></Section>
    </div>
  );
}
